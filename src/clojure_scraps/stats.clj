...
(def stat-file-folder "out-files/acc-perc-100gen/")
...
(defn- best-strat-based-on-fitness
  [str1 str2]
  (if (> (:fitness-score str1) (:fitness-score str2)) str1 str2))
...
(defn compare-training-and-test-performance
  ...
  [evolution-id & {:keys [all]
                   :or {all false}}]
  (let [strategies (get-strategies-of-evolution evolution-id)
        best-strat (reduce best-strat-based-on-fitness strategies)]
    (when all (log/info (pp/pprint (map (fn [strategy] (str "strategy-id: " (:guid strategy)
...
;(compare-training-and-test-performance (first res/accuracy-perc-100pop-100gen-4height-ids))
